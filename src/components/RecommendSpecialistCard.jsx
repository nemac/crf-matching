...
export default function RecommendSpecialistCard({
  specialist,
  urlFilters,
  index,
}) {
    ...
    >
      ...
      <Box
        sx={{
          ...
          minHeight: { xs: '70px', sm: '70px', md: '60px' },
        }}
      >
        <Typography
          variant="h7"
      ...
      >
        ...
        {isClamped && (
          ...
          >
            {expanded ? 'Less' : 'More'}
            {expanded ? (
              <KeyboardArrowUpIcon sx={{ fontSize: '0.65rem' }} />
            ) : (
              <KeyboardArrowDownIcon sx={{ fontSize: '0.65rem' }} />
            )}
          </Button>
        )}
      </Box>
      <Box sx={{ display: 'flex', flexWrap: 'wrap', p: 1 }}>
        <Box sx={{ width: '100%', mt: 0.5, color: theme.palette.purple }}>
